...
describe('subset', function (): void {
  ...
  it('should get the right subset of an array of booleans', function (): void {
    ...
    assert.deepStrictEqual(subset([math.bignumber(2)], index([true])), [
      math.bignumber(2)
    ])
  })
  ...
  it('should return an empty value with an empty index', function (): void {
    ...
    assert.deepStrictEqual(
      subset(b, index(new math.Range(0, 0), 1)),
      math.matrix()
    )
  ...
  it('should get the right subset of an array of booleans in the parser', function (): void {
    ...
    assert.deepStrictEqual(
      math.evaluate('[bignumber(2)][[true]]'),
      math.matrix([math.bignumber(2)])
    )
  })
  ...
  it('should set the right subset of an array if the replacement can be broadcasted to the index', function (): void {
    ...
    assert.deepStrictEqual(subset(d, index(new Range(0, 2), 1), -2), [
      [1, -2],
      [3, -2]
    ])
    assert.deepStrictEqual(d, [
    ...
    assert.deepStrictEqual(subset(d, index(2, new Range(0, 2)), [5]), [
      [1, 2],
      [3, 4],
      [5, 5]
    ])
    ...
    assert.deepStrictEqual(subset(d, index(0, [0, 1]), 123), [
      [123, 123],
      [3, 4]
    ])
  })
  ...
  it("should set the right subset of an array or matrix with default value if the replacement can't be broadcasted to the index", function (): void {
    ...
    assert.deepStrictEqual(subset(d, index(1, 2), 7, 0), [
      [1, 2, 0],
      [3, 4, 7]
    ])
    assert.deepStrictEqual(
      subset(g, index(1, 2), 7, 0),
      math.matrix([
        [1, 2, 0],
        [3, 4, 7]
      ])
  ...
  it('should set a subset of an array or matrix by broadcasting the replacement', function (): void {
    ...
    assert.deepStrictEqual(
      subset(g, index(new Range(0, 2), 1), -2),
      math.matrix([
        [1, -2],
        [3, -2]
      ])
  ...
  it('should work with config legacySubset during deprecation', function (): void {
    ...
    assert.deepStrictEqual(math2.subset(A, index(1, [2])), 6)
    assert.deepStrictEqual(math2.subset(A, index([1], [2])), 6)
    assert.deepStrictEqual(math2.subset(A, index(1, [1, 2])).toArray(), [
      [5, 6]
    ])
    assert.deepStrictEqual(math2.subset(A, index([0, 1], 1)).toArray(), [
      [2],
      [5]
    ])
