...
*/
{
  ...
  // createUnit
  // Note: `furlong` is a built-in unit in this fork, so we pass `override: true`
  // to avoid a runtime "unit already exists" error while still exercising the type.
  expectTypeOf(
    math.chain(math.createUnit('furlong', '220 yards', { override: true }))
...
*/
{
  ...
  */
  ...
  // creating units
  math.createUnit('foo')
  // `furlong` is a built-in unit in this fork; use override to avoid a collision.
