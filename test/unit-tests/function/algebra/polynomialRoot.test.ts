...
describe('polynomialRoot', function () {
  ...
  it('should solve a quadratic with two distinct roots', function () {
    ...
    approxDeepEqual(pRoot(4, 2, 1), [
      complex(-1, math.sqrt(3)),
      complex(-1, -math.sqrt(3))
    ])
    approxDeepEqual(pRoot(complex(3, 1), -3, 1), [
      complex(2, -1),
      complex(1, 1)
    ])
  })
  ...
  it('should solve a cubic with one simple and one double root', function () {
    ...
    approxDeepEqual(pRoot(complex(0, 6), complex(6, 8), complex(5, 2), 1), [
      -3,
      complex(-1, -1)
    ])
    approxDeepEqual(pRoot(complex(2, 6), complex(8, 6), complex(5, 1), 1), [
      complex(-3, 1),
      complex(-1, -1)
    ])
  })
