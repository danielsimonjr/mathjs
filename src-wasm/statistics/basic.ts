 ...
 * Note: Input data array will be modified (sorted)
 * @param data Input array (will be sorted)
 * @param length Length of data array
 * @param probs Array of probabilities (0 to 1)
...
export function quantileSeq(
  ...
  for (let i: i32 = 0; i < numProbs; i++) {
    unchecked(result[i] = quantile(data, length, unchecked(probs[i])))
  }
...
export function zscore(data: Float64Array, length: i32): Float64Array {
  ...
  for (let i: i32 = 0; i < length; i++) {
    unchecked(result[i] = (unchecked(data[i]) - m) / s)
  }
...
/**
 * Calculate median of unsorted array (creates sorted copy)
...
export function medianUnsorted(data: Float64Array, length: i32): f64 {
  ...
  for (let i: i32 = 0; i < length; i++) {
    unchecked(sorted[i] = unchecked(data[i]))
  }

  // Sort and compute median
  quicksort(sorted, 0, length - 1)
  return median(sorted, length)
}
 ...
 */
export function weightedMean(data: Float64Array, weights: Float64Array, length: i32): f64 {
  if (length === 0) return NaN
  ...
  for (let i: i32 = 0; i < length; i++) {
    const w = unchecked(weights[i])
    sumWeighted += unchecked(data[i]) * w
    sumWeights += w
  }
...
export function meanAbsoluteDeviation(data: Float64Array, length: i32): f64 {
  ...
  for (let i: i32 = 0; i < length; i++) {
    sumAbsDev += Math.abs(unchecked(data[i]) - m)
  }

  return sumAbsDev / f64(length)
}

/**
 * Calculate coefficient of variation (CV = std/mean)
 * @param data Input array (should be positive values)
 * @param length Length of array
