...
describe('map', function () {
  ...
  it('should map two arrays', function () {
    ...
    const callback = function (valueA, valueB) {
    ...
    const arr3 = math.map(
      arrA,
      arrB,
      math.typed({ 'number, number': callback })
    )
  ...
  it('should map three arrays', function () {
    ...
    const callback = function (valueA, valueB, valueC) {
      return valueA * 2 + valueB + valueC
    }
    ...
    const arr3 = math.map(
      ...
      arrB,
      arrC,
      math.typed({ 'number, number, number': callback })
    )
    assert.deepStrictEqual(arr3, expected)
    assert.ok(Array.isArray(arr3))
  })
  ...
  it('should map three arrays with broadcasting', function () {
    ...
    const arr2 = math.map(arrA, arrB, arrC, function (valueA, valueB, valueC) {
      return valueA * 2 + valueB + valueC / 2
    })
    assert.deepStrictEqual(arr2, [
      [62, 114, 166],
      [72, 124, 176],
  ...
  it('should map two matrices', function () {
    ...
    const mat2 = math.map(matA, matB, function (valueA, valueB) {
      return valueA * 2 + valueB
    })
  ...
  it('should invoke a typed function with correct number of arguments (4)', function () {
    ...
    assert.deepStrictEqual(
      math.map(math.matrix([1, 8, 27]), math.cbrt),
      math.matrix([1, 2, 3])
    )
    assert.deepStrictEqual(
      math.map(math.matrix([1, 8, 27], 'sparse'), math.cbrt),
      math.matrix([1, 2, 3], 'sparse')
    )
  })
  ...
  it('should invoke a typed function with correct number of arguments (5)', function () {
    ...
    assert.deepStrictEqual(
      math.map(math.matrix([1, 8, 27], 'sparse'), math.format),
      math.matrix(['1', '8', '27'], 'sparse')
    )
  })
  ...
  it('should operate from the parser with multiple inputs', function () {
    ...
    assert.deepStrictEqual(
      math.evaluate(
        'map([[1, 2, 3], [4, 5, 6]], [[10, 20, 30], [40, 50, 60]], f(a, b) = a * 2 + b)'
      ),
      math.matrix([
        [12, 24, 36],
        [48, 60, 72]
  ...
  it('should operate from the parser with multiple inputs that need broadcasting and one based indices and the broadcasted arrays', function () {
    ...
    const arr2 = math.evaluate(
      'map([1],[3,4], f(a,b,idx,A,B)= a + A[idx[1]] + b + B[idx[1]] + idx[1])'
    )
