...
describe('ceil', function () {
  ...
  it('should return the ceil of a big number with a given number of decimals', function () {
    ...
    assert.deepStrictEqual(
      ceil(bignumber(-1.315), bignumber(2)),
      bignumber(-1.31)
    )
  ...
  it('should return the ceil of real and imag part of a complex with a given number of decimals', function () {
    ...
    approxDeepEqual(
      ceil(complex(1.315, 1.878), bignumber(2)),
      complex(1.32, 1.88)
    )
  ...
  it('should ceil units', function () {
    ...
    assert.deepStrictEqual(
      ceil(unit('3.12345 cm'), 3, unit('cm')),
      unit('3.124 cm')
    )
    ...
    // bignumber values
    assert.deepStrictEqual(
      ceil(unit('3.12345 cm'), bignumber(2), unit('cm')),
      unit('3.13 cm')
    )
    assert.deepStrictEqual(
      ceil(unit(bignumber('2'), 'inch'), unit('cm')),
      unit(bignumber('6'), 'cm')
    )
    assert.deepStrictEqual(
      ceil(unit(bignumber('2'), 'inch'), bignumber(1), unit('cm')),
      unit(bignumber('5.1'), 'cm')
    )

    // first argument is a collection
    assert.deepStrictEqual(ceil([unit('2 inch'), unit('3 inch')], unit('cm')), [
      unit('6 cm'),
      unit('8 cm')
    ])
    assert.deepStrictEqual(
      ceil(matrix([unit('2 inch'), unit('3 inch')]), unit('cm')),
      matrix([unit('6 cm'), unit('8 cm')])
    )
  })
  ...
  it('should ceil each element in a matrix with a given number of decimals', function () {
    ...
    approxDeepEqual(
      ceil(matrix([1.282, 3.415, -5.121, -10.128]), bignumber(2)),
      matrix(bignumber([1.29, 3.42, -5.12, -10.12]))
    )
  })
