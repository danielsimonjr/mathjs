...
export var createMatrixFromRows = /* #__PURE__ */factory(name, dependencies, _ref => {
  ...
  return typed(name, {
    ...
    '...': function _(arr) {
      ...
      var allMatrix = arr.every(item => typeof item.toArray === 'function');
      // Check if any argument is a plain array
      var hasArray = arr.some(item => Array.isArray(item));

      // Convert all to arrays for processing
      var arrays = arr.map(item => typeof item.toArray === 'function' ? item.toArray() : item);
      var result = _createArray(arrays);
