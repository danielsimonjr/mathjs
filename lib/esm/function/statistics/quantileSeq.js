...
export var createQuantileSeq = /* #__PURE__ */factory(name, dependencies, _ref => {
  ...
  function _quantileSeq(array, prob, sorted) {
    ...
    if (isNumber(prob) && bignumber) {
      var hasBigNumber = flat.some(x => isBigNumber(x));
      if (hasBigNumber) {
    ...
    var index = isNumber(actualProb) ? actualProb * (len - 1) : actualProb.times(len - 1);
    var integerPart = isNumber(actualProb) ? Math.floor(index) : index.floor().toNumber();
    var fracPart = isNumber(actualProb) ? index % 1 : index.minus(integerPart);
    if (isInteger(index)) {
      return sorted ? flat[index] : partitionSelect(flat, isNumber(actualProb) ? index : index.valueOf());
    }
    var left;
    var right;
    if (sorted) {
