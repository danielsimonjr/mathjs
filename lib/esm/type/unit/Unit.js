...
export var createUnitClass = /* #__PURE__ */factory(name, dependencies, _ref => {
  ...
  function normalizeUnitKey(unitObj) {
    var normalizedName = normalizeUnitName(unitObj.unit.name);
    var prefixName = unitObj.prefix ? unitObj.prefix.name : '';
    return "".concat(normalizedName, "_").concat(prefixName);
  }
  ...
  function simplifyUnit(unit) {
    ...
    var simplifiedUnits = units.filter(u => Math.abs(u.power) >= 1e-12);
