...
export var createSparseMatrixClass = /* #__PURE__ */factory(name, dependencies, _ref => {
  ...
  SparseMatrix.prototype.reshape = function (sizes, copy) {
    ...
    for (var _i2 = 0; _i2 < m._index.length; _i2++) {
      var r1 = rowIndex[_i2];
      var c1 = colIndex[_i2];
    ...
    m._size = sizes.slice();
    for (var _i3 = 0; _i3 < m._ptr.length; _i3++) {
      m._ptr[_i3] = 0;
    }
    ...
    for (var h = 0; h < values.length; h++) {
      ...
      var _j = colIndex[h];
      var v = values[h];
      var k = _getValueIndex(_i4, m._ptr[_j], m._ptr[_j + 1], m._index);
      _insert(k, _i4, _j, v, m._values, m._index, m._ptr);
    }
    ...
    for (var j = minColumn; j <= maxColumn; j++) {
      ...
      if (skipZeros) {
        ...
        for (var _k = k0; _k < k1; _k++) {
          var _i5 = matrix._index[_k];
          _values[_i5] = matrix._values[_k];
        }

        // loop over all rows (indexes can be unordered so we can't use that),
        // and either read the value or zero
        for (var _i6 = minRow; _i6 <= maxRow; _i6++) {
          var value = _i6 in _values ? _values[_i6] : 0;
          invoke(value, _i6 - minRow, j - minColumn);
        }
      }
    }
    ...
    for (var j = 0; j < columns; j++) {
      ...
      if (skipZeros) {
        ...
        for (var _k2 = k0; _k2 < k1; _k2++) {
          var _i7 = this._index[_k2];
          values[_i7] = this._values[_k2];
        }

        // loop over all rows (indexes can be unordered so we can't use that),
        // and either read the value or zero
        for (var _i8 = 0; _i8 < rows; _i8++) {
          var value = _i8 in values ? values[_i8] : 0;
          fastCallback.fn(value, [_i8, j], me);
        }
      }
    }
  };
