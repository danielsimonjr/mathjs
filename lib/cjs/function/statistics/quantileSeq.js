...
const createQuantileSeq = exports.createQuantileSeq = /* #__PURE__ */(0, _factory.factory)(name, dependencies, _ref => {
  ...
  function _quantileSeq(array, prob, sorted) {
    ...
    let actualProb = prob;
    if ((0, _is.isNumber)(prob) && bignumber) {
      const hasBigNumber = flat.some(x => (0, _is.isBigNumber)(x));
      if (hasBigNumber) {
    ...
    const index = (0, _is.isNumber)(actualProb) ? actualProb * (len - 1) : actualProb.times(len - 1);
    const integerPart = (0, _is.isNumber)(actualProb) ? Math.floor(index) : index.floor().toNumber();
    const fracPart = (0, _is.isNumber)(actualProb) ? index % 1 : index.minus(integerPart);
    if (isInteger(index)) {
      return sorted ? flat[index] : partitionSelect(flat, (0, _is.isNumber)(actualProb) ? index : index.valueOf());
    }
    let left;
    let right;
    if (sorted) {
