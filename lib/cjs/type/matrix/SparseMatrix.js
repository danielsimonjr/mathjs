...
const createSparseMatrixClass = exports.createSparseMatrixClass = /* #__PURE__ */(0, _factory.factory)(name, dependencies, _ref => {
  ...
  function _createFromArray(matrix, data, datatype) {
    ...
    // Check for 1D arrays
    if (rows > 0 && !(0, _is.isArray)(data[0])) {
      throw new _DimensionError.DimensionError('Two dimensional array expected');
    }
    ...
    for (let i = 0; i < rows; i++) {
      ...
      if ((0, _is.isArray)(row)) {
        ...
        if (expectedColumns === null) {
          ...
          throw new _DimensionError.DimensionError(row.length, expectedColumns);
        }
      }
    }
