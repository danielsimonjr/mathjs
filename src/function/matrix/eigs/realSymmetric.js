...
export function createRealSymmetric ({ config, addScalar, subtract, abs, atan, cos, sin, multiplyScalar, inv, bignumber, multiply, add }) {
  ...
  // with Decimal.js because valueOf() returns strings (lexicographic, not numeric).
  function bigGte (a, b) {
  ...
  function diagBig (x, precision, computeVectors) {
    ...
    if (computeVectors) {
      Sij = new Array(N)
      // Sij is Identity Matrix — must use BigNumber values
      for (let i = 0; i < N; i++) {
        Sij[i] = new Array(N)
        for (let j = 0; j < N; j++) {
  ...
  function getThetaBig (aii, ajj, aij) {
    ...
    if (bigLte(abs(denom), bignumber(config.relTol))) {
      ...
      return multiplyScalar(bignumber(0.5), atan(multiply(bignumber(2), aij, inv(denom))))
    }
  }
