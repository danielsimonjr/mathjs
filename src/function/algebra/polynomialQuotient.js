...
const name = 'polynomialQuotient'
const dependencies = ['typed', 'parse', 'simplify']

export const createPolynomialQuotient = /* #__PURE__ */ factory(name, dependencies, ({
  typed,
  parse,
  simplify
}) => {
  /**
   * Compute the quotient of polynomial long division p / q.
   *
   * Divides polynomial p by polynomial q and returns the quotient polynomial.
   * Both polynomials must be univariate in the same variable.
   *
   * Syntax:
   *
   *     math.polynomialQuotient(p, q, variable)
   *
   * Examples:
   *
   *     math.polynomialQuotient('x^3 - 1', 'x - 1', 'x')           // 'x ^ 2 + x + 1'
   *     math.polynomialQuotient('x^2 + 3*x + 2', 'x + 1', 'x')     // 'x + 2'
   *     math.polynomialQuotient('x^3 + x^2 - x - 1', 'x^2 - 1', 'x')  // 'x + 1'
   *
   * See also:
   *
   *     polynomialRemainder, polynomialGCD, simplify
   *
   * @param {Node|string} p      Dividend polynomial
   * @param {Node|string} q      Divisor polynomial
   * @param {string} variable    The variable name
   * @return {string}            The quotient polynomial as a string
   */
  return typed(name, {
    'Node, Node, string': function (p, q, variable) {
      return dividePolynomials(p, q, variable).quotient
    },
    'string, string, string': function (p, q, variable) {
      return dividePolynomials(parse(p), parse(q), variable).quotient
    },
    'string, Node, string': function (p, q, variable) {
      return dividePolynomials(parse(p), q, variable).quotient
    },
    'Node, string, string': function (p, q, variable) {
      return dividePolynomials(p, parse(q), variable).quotient
    }
  })
  ...
  function extractCoeffs (node, variable) {
    ...
    for (let i = 0; i <= MAX_DEGREE + 1; i++) {
      ...
      try {
        ...
        throw new Error('polynomialQuotient: failed to evaluate polynomial: ' + e.message)
      }
      if (typeof val !== 'number' || !isFinite(val)) {
        throw new Error('polynomialQuotient: expression must be a numeric polynomial')
      }
      values.push(val)
    }
