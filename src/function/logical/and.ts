...
export const createAnd = /* #__PURE__ */ factory(name, dependencies, ({ typed, matrix, equalScalar, zeros, not, concat }: Dependencies) => {
  ...
  return typed(
    name,
    {
      ...
      'SparseMatrix, any': (typed as any).referToSelf((self: any) => (x: Matrix, y: any): any => {
        // check scalar
        if (not(y)) {
          // return zero matrix
      ...
      'DenseMatrix, any': (typed as any).referToSelf((self: any) => (x: Matrix, y: any): any => {
        // check scalar
        if (not(y)) {
          // return zero matrix
          return zeros(x.size(), x.storage())
      ...
      'any, SparseMatrix': (typed as any).referToSelf((self: any) => (x: any, y: Matrix): any => {
        // check scalar
        if (not(x)) {
          // return zero matrix
          return zeros(x.size(), x.storage())
      ...
      'any, DenseMatrix': (typed as any).referToSelf((self: any) => (x: any, y: Matrix): any => {
        // check scalar
        if (not(x)) {
          // return zero matrix
          return zeros(x.size(), x.storage())
      ...
      'Array, any': (typed as any).referToSelf((self: any) => (x: any[], y: any): any => {
        // use matrix implementation
        return self(matrix(x), y).valueOf()
      }),

      'any, Array': (typed as any).referToSelf((self: any) => (x: any, y: any[]): any => {
        // use matrix implementation
